...
@Component({
  selector: 'app-agro-firma-egresos-cuotas',
  templateUrl: './agro-firma-egresos-cuotas.component.html',
  styleUrls: ['./agro-firma-egresos-cuotas.component.scss']
})
export class AgroFirmaEgresosCuotasComponent implements OnInit {
  ...
  dataCuota: EgresoAgroFirmaCuota[] = [];
  ...
  obtenerCuotas(){
    this.agroFirmaService.getCuotas(this.idEgreso)
    .pipe()
    .subscribe((x:any) => {      
  ...
  procesarPago(){
   ...
   const dialogRef = this.dialog.open(DialogRespaldosComponent, {
      data: { url: 'egresoagroFirmaCuota/upload' }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {     
      ...
      this.agroFirmaService.agregarRespaldos(arrayRespaldos).subscribe(
        (data: any) => {         
  ...
  recuperarArchivos(archivos: any) {     
    if(archivos.estadoCuota == "Pendiente"){
      ...
      this.agroFirmaService.buscarImagenC(archivos.id).subscribe(
        (dataImagen: any) => {                  
          setTimeout(() => {
            this.dialog.open(DialogShow, {
              data: { archivos: dataImagen, servicio: 'agroFirma-egreso-cuota' },
            });
          }, 1000);       
        },
        (error: any) => {
  ...
  edit(id: any, monto: any, cuota: any) {
    ...
    if(cuota == "Pendiente"){
      this.agroFirmaService.openDialogCuota();
    }else {    
