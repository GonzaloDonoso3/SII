...
@Component({
  selector: 'app-abogados-egresos-cuotas',
  templateUrl: './abogados-egresos-cuotas.component.html',
  styleUrls: ['./abogados-egresos-cuotas.component.scss']
})
export class AbogadosEgresosCuotasComponent implements OnInit {
  ...
  obtenerCuotas(){    
    this.abogadosService.getCuotas(this.idEgreso)
    .pipe()
    .subscribe((x:any) => {      
  ...
  procesarPago(){
   ...
   const dialogRef = this.dialog.open(DialogRespaldosComponent, {
      data: { url: 'egresoFirmaCuota/upload' }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {     
      ...
      this.abogadosService.agregarRespaldos(arrayRespaldos).subscribe(
        (data: any) => {         
  ...
  recuperarArchivos(archivos: any) {     
    if(archivos.estadoCuota == "Pendiente"){
      ...
      this.abogadosService.buscarImagenC(archivos.id).subscribe(
        (dataImagen: any) => {                  
          setTimeout(() => {
            this.dialog.open(DialogShow, {
              data: { archivos: dataImagen, servicio: 'abogados-egreso-cuota' },
            });
          }, 1000);       
        },
        (error: any) => {
  ...
  edit(id: any, monto: any, cuota: any) {
    ...
    if(cuota == "Pendiente"){
      this.abogadosService.openDialogCuota();
    }else {    
