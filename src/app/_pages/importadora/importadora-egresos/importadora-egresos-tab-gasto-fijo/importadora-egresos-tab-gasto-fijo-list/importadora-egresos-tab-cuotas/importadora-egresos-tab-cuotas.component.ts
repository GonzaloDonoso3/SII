...
@Component({
  selector: 'app-importadora-egresos-tab-cuotas',
  templateUrl: './importadora-egresos-tab-cuotas.component.html',
  styleUrls: ['./importadora-egresos-tab-cuotas.component.scss']
})
export class ImportadoraEgresosTabCuotasComponent implements OnInit {
  ...
  dataSource: MatTableDataSource<EgresoFijoImportadoraCuota> = new MatTableDataSource();  
  dataCuota: EgresoFijoImportadoraCuota[] = [];
  ...
  obtenerCuotas(){
    this.importadoraService.getCuotas(this.idEgreso)
    .pipe()
    .subscribe((x:any) => {      
  ...
  procesarPago(){
   ...
   const dialogRef = this.dialog.open(DialogRespaldosComponent, {
      data: { url: 'egresoFijoImportadoraCuota/upload' }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {     
      ...
      this.importadoraService.agregarRespaldos(arrayRespaldos).subscribe(
        (data: any) => {         
  ...
  recuperarArchivos(archivos: any) {     
    if(archivos.estadoCuota == "Pendiente"){
      ...
      this.importadoraService.buscarImagenC(archivos.id).subscribe(
        (dataImagen: any) => {                  
          setTimeout(() => {
            this.dialog.open(DialogShow, {
              data: { archivos: dataImagen, servicio: 'importadora-egresoFijo-cuota' },
            });
          }, 1000);       
        },
        (error: any) => {
  ...
  edit(id: any, monto: any, cuota: any) {
    ...
    if(cuota == "Pendiente"){
      this.importadoraService.openDialogCuota();
    }else {    
